...
export default function footerTokenProgress(pi: ExtensionAPI) {
	pi.on("session_start", (_event, ctx) => {
		ctx.ui.setFooter((tui, theme, footerData) => {
			let totals = getAssistantUsageTotals(ctx.sessionManager.getBranch());

			return {
				dispose: footerData.onBranchChange(() => {
					totals = getAssistantUsageTotals(ctx.sessionManager.getBranch());
					tui.requestRender();
				}),
				invalidate() {},
				render(width: number): string[] {
					...
					const leftParts = [formatLeftStatus(ctx.cwd, footerData.getGitBranch(), theme, () => tui.requestRender())];
